...
class BuyMenu:
    ...
    def draw(self, surface, font, font_small):
        ...
        # Titolo compatto in alto a sinistra
        title = font.render(f"ARMERIA - {self.territory.name.upper()}", True, (255, 255, 100))
        surface.blit(title, (self.x + 20, self.y + 10))
        
        # Soldi in alto a destra
        money = FACTIONS[self.faction]["money"]
        money_text = font.render(f"[$] ${money}", True, (100, 255, 100))
        surface.blit(money_text, (self.x + self.width - 200, self.y + 10))
        ...
        oil_prod = font_small.render(f"[P] {self.territory.oil_production}P", True, (255, 200, 100))
        surface.blit(oil_prod, (self.x + 135, self.y + 155))
        
        tech_prod = font_small.render(f"[T] {self.territory.tech_points}T", True, (150, 150, 255))
        surface.blit(tech_prod, (self.x + 240, self.y + 155))
        
        # Unità presenti con ICONE
        ...
        # Fanteria con icona
        self.draw_unit_icon(surface, "fanteria", self.x + 15, self.y + 195, 22)
        inf_text = font_small.render(f"x{counts['fanteria']}", True, (255, 255, 255))
        surface.blit(inf_text, (self.x + 40, self.y + 200))
        
        # Carri con icona
        self.draw_unit_icon(surface, "carro", self.x + 75, self.y + 195, 22)
        tank_text = font_small.render(f"x{counts['carro']}", True, (255, 255, 255))
        surface.blit(tank_text, (self.x + 100, self.y + 200))
        
        # Aerei con icona
        self.draw_unit_icon(surface, "aereo", self.x + 135, self.y + 195, 22)
        plane_text = font_small.render(f"x{counts['aereo']}", True, (255, 255, 255))
        surface.blit(plane_text, (self.x + 160, self.y + 200))
        
        # Bombardieri con icona
        self.draw_unit_icon(surface, "bombardiere", self.x + 195, self.y + 195, 22)
        bomber_text = font_small.render(f"x{counts['bombardiere']}", True, (255, 255, 255))
        surface.blit(bomber_text, (self.x + 220, self.y + 200))
        
        # Totale
        total_units = len(self.territory.units)
        total_text = font_small.render(f"TOT: {total_units}", True, (255, 215, 100))
        surface.blit(total_text, (self.x + 270, self.y + 200))
        
        # DIFESE STRUTTURALI (se presenti)
        if self.territory.bunkers > 0 or self.territory.towers > 0 or self.territory.fortress > 0:
            ...
            def_text = font_small.render(f"DIFESE: +{def_bonus} (B:{self.territory.bunkers} T:{self.territory.towers} F:{self.territory.fortress})", 
                                         True, (255, 200, 150))
            surface.blit(def_text, (self.x + 25, self.y + 225))
        
        # Separatore
        pygame.draw.line(surface, (100, 100, 150), 
                        (self.x + 15, self.y + 240), 
                        (self.x + self.width - 15, self.y + 240), 2)
        
        # ===== SEZIONE ACQUISTA =====
        buy_title = font_small.render("ACQUISTA ARMAMENTI:", True, (255, 215, 100))
        surface.blit(buy_title, (self.x + 20, self.y + 250))
        
        # Pulsanti acquisto
        y_offset = 270
        for i, unit_type in enumerate(["fanteria", "carro", "aereo", "bombardiere"]):
            stats = UNIT_STATS[unit_type]
