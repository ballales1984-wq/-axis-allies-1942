...
class Territory:
    ...
    def draw(self, surface, font_small):
        """Disegna territorio PULITO (solo puntino)"""
        ...
        # SOLO indicatore sviluppo se avanzato (>= 5)
        if self.development_level >= 5:
            dev_text = font_small.render(f"★{self.development_level}", True, (255, 215, 0))
            dev_rect = dev_text.get_rect(center=(self.x, self.y - 12))
            bg_dev = pygame.Surface((dev_rect.width + 4, dev_rect.height + 2))
            bg_dev.set_alpha(220)
            bg_dev.fill((0, 0, 0))
            surface.blit(bg_dev, (dev_rect.x - 2, dev_rect.y - 1))
...
class BuyMenu:
    ...
    def __init__(self, territory, faction, turn):
        ...
        self.height = 560  # Più alto per sezione unità presenti
    ...
    def draw(self, surface, font, font_small):
        ...
        # ===== SEZIONE UNITÀ PRESENTI =====
        present_bg = pygame.Surface((self.width - 20, 70))
        present_bg.fill((40, 60, 40))
        surface.blit(present_bg, (self.x + 10, self.y + 95))
        
        # Titolo sezione
        present_title = font_small.render("UNITÀ PRESENTI IN CAMPO:", True, (100, 255, 100))
        surface.blit(present_title, (self.x + 20, self.y + 100))
        
        # Conta unità presenti
        counts = self.territory.count_units_by_type()
        
        # Mostra dettaglio
        y_unit = self.y + 120
        if counts["fanteria"] > 0:
            f_text = font_small.render(f"👥 Fanteria: {counts['fanteria']}", True, (255, 255, 255))
            surface.blit(f_text, (self.x + 25, y_unit))
            y_unit += 15
        
        if counts["carro"] > 0:
            c_text = font_small.render(f"🚜 Carri: {counts['carro']}", True, (255, 255, 255))
            surface.blit(c_text, (self.x + 25, y_unit))
            y_unit += 15
        
        if counts["aereo"] > 0:
            a_text = font_small.render(f"✈️ Aerei: {counts['aereo']}", True, (255, 255, 255))
            surface.blit(a_text, (self.x + 25, y_unit))
            y_unit += 15
        
        if counts["fanteria"] == 0 and counts["carro"] == 0 and counts["aereo"] == 0:
            empty_text = font_small.render("Nessuna unità", True, (150, 150, 150))
            surface.blit(empty_text, (self.x + 25, self.y + 120))
...
class Game:
    ...
    def show_territory_info(self):
        ...
        msg = f"{t.name}: F:{counts['fanteria']} C:{counts['carro']} A:{counts['aereo']} | ATT:{att} DEF:{deff} | ${t.income} {t.oil_production}P {t.tech_points}T | {dev_info}"
        self.show_message(msg)
