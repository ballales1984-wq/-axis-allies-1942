...
class Territory:
    ...
    def __init__(self, data):
        ...
        self.income = random.randint(100, 500)  # Soldi
        self.oil_production = random.randint(50, 200)  # Petrolio
        self.tech_points = random.randint(10, 50)  # Tecnologia
    ...
    def get_total_defense(self, tech_bonus=0):
        """Potenza difesa totale con BONUS TECNOLOGIA"""
        base_defense = sum(u.defense for u in self.units if not u.needs_repair)
        return base_defense + tech_bonus
...
class BuyMenu:
    ...
    def draw(self, surface, font, font_small):
        ...
        # Help alla fine
        help_text = font_small.render("🖱️ CLICCA sui pulsanti | Trascina barra | ESC=Chiudi", True, (150, 150, 200))
...
class Game:
    ...
    def __init__(self):
        ...
        # Sistema IA - solo ROSSI è umano, gli altri sono PC
        self.human_faction = "rossi"
        self.ai_factions = ["blu", "verdi", "gialli", "viola"]
        ...
        print("="*70)
        print("AXIS & ALLIES 1942 - GIOCATORE vs PC")
        print("="*70)
        print("TU controlli: ROSSI")
        print("PC controlla: BLU, VERDI, GIALLI, VIOLA")
        print()
    ...
    def next_turn(self):
        ...
        self.show_message(f"{FACTIONS[self.current_faction]['name']} +${income} +{oil}P +{tech}T")
    ...
    def show_territory_info(self):
        ...
        msg = f"{t.name}: F:{counts['fanteria']} C:{counts['carro']} A:{counts['aereo']} | ATT:{att} DEF:{deff} | ${t.income} {t.oil_production}P {t.tech_points}T"
        self.show_message(msg)
    
    def attack(self, attacker, defender):
        """Sistema combattimento con 3 RISORSE e TECH"""
        ...
        # Check PETROLIO
        oil_cost = 20 * len(attacker.units)
        if FACTIONS[attacker.owner]["oil"] < oil_cost:
            self.show_message(f"Petrolio insufficiente! Servono {oil_cost}P")
            return
        
        # Consuma petrolio
        FACTIONS[attacker.owner]["oil"] -= oil_cost
        ...
        # Combattimento con TECH
        att_power = attacker.get_total_attack(att_tech["bonus_attack"])
        ...
        if len(defender.units) == 0:
            ...
            # Calcola BONUS conquista (basato su valore territorio)
            conquest_bonus = defender.income * 5  # 5 volte il reddito
            
            # Calcola PENALITÀ per chi perde (se non neutrale)
            if original_defender_owner is not None:
                loss_penalty = defender.income * 3  # 3 volte il reddito
                old_money = FACTIONS[original_defender_owner]["money"]
                FACTIONS[original_defender_owner]["money"] = max(0, old_money - loss_penalty)
                
                print(f"  💸 {FACTIONS[original_defender_owner]['name']} PERDE ${loss_penalty}!")
                
                # Messaggio se è il giocatore umano che perde
                if original_defender_owner == self.human_faction:
                    self.show_message(f"❌ PERSO {defender.name}! -${loss_penalty}")
            
            # Dai BONUS a chi conquista
            FACTIONS[attacker.owner]["money"] += conquest_bonus
            print(f"  💰 {FACTIONS[attacker.owner]['name']} GUADAGNA ${conquest_bonus}!")
            ...
            if attacker.owner == self.human_faction:
                self.show_message(f"✅ CONQUISTATO {defender.name}! +${conquest_bonus}")
            else:
                self.show_message(f"[IA] CONQUISTATO {defender.name}! +${conquest_bonus}")
        else:
            self.show_message(f"⚔️ Battaglia! Att:-{att_losses} Def:-{def_losses}")
            ...
            for event in pygame.event.get():
                ...
                # Timer IA
                    ...
                    if event.button == 1:
                        # Solo se è turno umano
                        if self.current_faction != self.human_faction:
                            self.show_message("Non è il tuo turno!")
                        elif self.buy_menu:
