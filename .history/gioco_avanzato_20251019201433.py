...
class Game:
    ...
    def attack(self, attacker, defender):
        """Sistema combattimento con RAGGIO VARIABILE"""
        ...
        # Se fuori portata normale, check missili
        if distance > attack_range:
            if attacker.has_missiles():
                # ATTACCO CON MISSILI!
                self.missile_attack(attacker, defender, distance)
                return
            else:
                self.show_message(f"FUORI PORTATA! Distanza:{int(distance)} Max:{attack_range}")
                return
        ...
        # Animazione battaglia (se vicino, altrimenti missile fatto separatamente)
        if distance <= attack_range:
    ...
    def missile_attack(self, attacker, defender, distance):
        ...
        if len(defender.units) == 0:
            ...
            self.show_message(f"[🚀] {defender.name} conquistato! +${money_bonus}")
        else:
            self.show_message(f"[🚀 -{oil_used}P] Missile! Att:-{att_losses} Def:-{def_losses}")
