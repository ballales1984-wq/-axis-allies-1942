...
class Territory:
    ...
    def draw(self, surface, font_small):
        """Disegna territorio con DETTAGLIO unità"""
        ...
        # DETTAGLIO unità per tipo (F:3 C:2 A:1)
        if len(self.units) > 0:
            counts = self.count_units_by_type()
            
            # Costruisci stringa compatta
            parts = []
            if counts["fanteria"] > 0:
                parts.append(f"F:{counts['fanteria']}")
            if counts["carro"] > 0:
                parts.append(f"C:{counts['carro']}")
            if counts["aereo"] > 0:
                parts.append(f"A:{counts['aereo']}")
            
            if parts:
                unit_text = " ".join(parts)
                text = font_small.render(unit_text, True, (255, 255, 255))
                rect = text.get_rect(topleft=(self.x + 10, self.y - 6))
