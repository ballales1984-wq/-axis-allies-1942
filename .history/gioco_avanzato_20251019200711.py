...
from effetti_visivi import ParticleSystem, BattleAnimation
...
class Territory:
    ...
    def draw(self, surface, font_small, graphics):
        """Disegna territorio con GRAFICA MIGLIORATA"""
        color = self.get_color()
        
        # GLOW se selezionato
        if self.selected:
            graphics.draw_pulsing_circle(surface, (self.x, self.y), 12, COLOR_SELECTED, self.id)
        
        # Puntino con gradiente e glow
        glow_intensity = 2 + self.development_level // 3
        graphics.draw_glowing_circle(surface, (self.x, self.y), self.radius, color, glow_intensity)
        
        # Bordo brillante
        pygame.draw.circle(surface, (255, 255, 255, 150), (self.x, self.y), self.radius, 1)
...
class Game:
    ...
    def attack(self, attacker, defender):
        """Sistema combattimento con RIFORNIMENTO AUTOMATICO"""
        ...
        # CALCOLA quante unità possono essere rifornite
        ...
        if units_with_fuel == 0:
            self.show_message("NESSUN PETROLIO! Attacco impossibile")
            return
        ...
        # Messaggio se attacco parziale
        if units_with_fuel < total_units:
            print(f"\n[PETROLIO] Solo {units_with_fuel}/{total_units} unità rifornite!")
            self.show_message(f"Attacco con {units_with_fuel}/{total_units} unità (-{oil_used}P)")
        ...
        # Combattimento - SOLO con unità rifornite!
        # Calcola potenza proporzionale
        if units_with_fuel < total_units:
            # Potenza ridotta proporzionalmente
            full_att_power = attacker.get_total_attack(att_tech["bonus_attack"])
            att_power = int(full_att_power * (units_with_fuel / total_units))
        else:
            # Potenza completa
        ...
        print(f"  Unità rifornite: {units_with_fuel}/{total_units} (-{oil_used}P)")
