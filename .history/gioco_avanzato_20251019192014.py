...
class BuyMenu:
    ...
    def draw(self, surface, font, font_small):
        ...
        # Bordo (giallo se trascinabile, verde se in dragging)
        border_color = (100, 255, 100) if self.dragging else (255, 215, 0)
        pygame.draw.rect(surface, border_color, (self.x, self.y, self.width, self.height), 3)
        
        # Barra titolo per trascinare (area cliccabile)
        title_bar_height = 40
        title_bar_color = (50, 50, 100) if not self.dragging else (70, 255, 70)
        pygame.draw.rect(surface, title_bar_color, (self.x, self.y, self.width, title_bar_height))
        
        # Titolo con icona drag
        drag_icon = font_small.render("≡≡", True, (150, 150, 150))
        surface.blit(drag_icon, (self.x + 5, self.y + 10))
        
        title = font.render(f"ARMERIA - {self.territory.name}", True, (255, 255, 100))
        surface.blit(title, (self.x + 30, self.y + 12))
        
        # Soldi disponibili
        money = FACTIONS[self.faction]["money"]
        money_text = font_small.render(f"Soldi: ${money}", True, (100, 255, 100))
        surface.blit(money_text, (self.x + 20, self.y + 55))
    ...
    def draw_unit_card(self, surface, font, font_small, unit_type, stats, x, y):
        """Disegna carta unità"""
        # Box unità (più stretto)
        card_width = 310
        pygame.draw.rect(surface, (50, 50, 80), (x, y, card_width, 100))
        pygame.draw.rect(surface, (100, 100, 150), (x, y, card_width, 100), 2)
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:
                        if self.buy_menu:
                            # Check se clicco sulla barra titolo per trascinare
                            if self.buy_menu.in_title_bar(event.pos[0], event.pos[1]):
