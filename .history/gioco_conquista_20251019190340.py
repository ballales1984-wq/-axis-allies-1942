...
class Territory:
    ...
    def __init__(self, data):
        ...
        self.units = 0
        self.selected = False
        self.radius = 4  # PUNTINI MICRO!
    
    def get_color(self):
        """Colore basato sullo STATO dalla mappa (non dalla fazione)"""
        # Usa il colore originale dello stato dalla mappa!
        return self.original_color
    
    def contains_point(self, px, py):
        """Check se punto è dentro (area cliccabile più grande del puntino)"""
        dist_sq = (px - self.x)**2 + (py - self.y)**2
        return dist_sq <= 100  # Area click più grande per facilità
    
    def draw(self, surface, font, font_small):
        """Disegna territorio come PUNTINO piccolo"""
        color = self.get_color()
        
        # Bordo oro più grande se selezionato
        if self.selected:
            pygame.draw.circle(surface, COLOR_SELECTED, (self.x, self.y), 16, 3)
        
        # PUNTINO colorato (molto piccolo!)
        pygame.draw.circle(surface, color, (self.x, self.y), self.radius)
        pygame.draw.circle(surface, (255, 255, 255), (self.x, self.y), self.radius, 1)
        
        # Nome SOLO se selezionato (altrimenti troppi testi)
        if self.selected:
        ...
        # Unità (numero accanto al puntino)
        if self.units > 0:
            unit_text = font_small.render(str(self.units), True, (255, 255, 255))
            unit_rect = unit_text.get_rect(center=(self.x + 15, self.y))
...
class Game:
    ...
    def load_background(self):
        """Carica mappa in BIANCO E NERO"""
        # Prima prova mappa B/N
        if os.path.exists("mappa_bn.jpg"):
            try:
                bg = pygame.image.load("mappa_bn.jpg")
                bg = pygame.transform.scale(bg, (SCREEN_WIDTH, SCREEN_HEIGHT))
                print("[OK] Sfondo BIANCO E NERO caricato")
                return bg
            except:
                pass
        
        # Altrimenti usa mappa normale
